    ...
    public override void OnRoomListUpdate(List<RoomInfo> roomList)
    {
        ...
        foreach(RoomInfo roomInfo in roomList)
        {
            ...
            if (roomInfo.Name.Contains(MultiplayerVRConstants.MAP_TYPE_VALUE_OUTDOOR))
            {
                ...
                Debug.Log("Room is an Outdoor map. Player count is: " + roomInfo.PlayerCount);
                OccupancyRateText_ForOutdoor.text = roomInfo.PlayerCount + " / " + 20;
            }
            else if (roomInfo.Name.Contains(MultiplayerVRConstants.MAP_TYPE_VALUE_SCHOOL))
            {
                //Update the School Room Occupancy Field
                Debug.Log("Room is an School map. Player count is: " + roomInfo.PlayerCount);
                OccupancyRateText_ForSchool.text = roomInfo.PlayerCount + " / " + 20;
            }
        }
    }
